...
pub fn stmt2ir(
    ...
    match stmt {
        ...
        Stmt::Assign(left, right) => {
            let (mut left_ir, left_type) = left_value2ir(left, scope,
                                                         source_name, source);
            let right_ir = expr2ir(right, scope, left_type.clone(),
                                   source_name, source);
            // needs first address, then value
            // store needs to be generated by `left_value2ir`
            let store_ir = left_ir.pop().unwrap_or((IRType::Error, IR::Error));

            left_ir.extend(right_ir);
            left_ir.push(store_ir);
            left_ir
        }
